   ...
   //     var speed = 5210; // 50m/50ms, 1000m/s 标准
       ...
       var getNextPoint = function(currentPoint, stopPoint) {
        ...
        var renderDyn=function (event) {
             ...
             */
             //由原84坐标计算而来的数据进行偏移3587
            var gcj=  GPSJP.gcj_encrypt(Number(nextPoint[1]),Number(nextPoint[0]));
            var tc=ol.proj.fromLonLat([Number(gcj.lon),Number(gcj.lat)],"EPSG:3857");
            //由原84坐标计算而来的数据进行偏移3587
            //targetFeature[0].getGeometry().appendCoordinate(nextPoint);
            targetFeature[0].getGeometry().appendCoordinate(tc);
            ...
            if(currentPoint[0]==stopPoint[0]&&currentPoint[1]==stopPoint[1]) {
                ...
                var originalwgslast=window._original84XY[window._original84XY.length-1];
                if(stopPoint[0]==Number(originalwgslast.lat)&&stopPoint[1]==Number(originalwgslast.long)){
                    debugger
                    ...
                    for (var i =   window.map.getLayers().getLength() - 1; i >= 0; i--) {
                       ...
                       if(tlyr.get('name')=="dynamic2qz"){
                         ...
                         tlyr.getSource().addFeature(multiptFeaturesz);
                           infotime.historypathtime(window.map, "endtime", last,_endtime,beginliesrighttoend);
                        }
                    }
                    return;
                }
...
*/
                ...
                startPointIndex++;
               // stopPoint = mainhistoryXYarray[startPointIndex + 1]; 由于原wgs84坐标需用于计算，暂时注释
                stopPoint =[Number(window._original84XY[startPointIndex+1].lat),Number(window._original84XY[startPointIndex+1].long)];
            }
            //pmap.render();
...
*/
            ...
            multiptFeaturesz.setStyle(styles.endicon);
             beginliesrighttoend=false;
            if(Number(original84XY[0].lat)>Number(original84XY[original84XY.length-1].lat))
            {
                beginliesrighttoend=true;
            }
           ...
           // vectorLayerHistoryVertxMainPt.setStyle(styles.middleicon);
            ...
            infotime.historypathtime(window.map, "begintime", mainhistoryXYarray[0],_begintime,beginliesrighttoend);
...
*/
            startPoint = [Number(original84XY[startPointIndex].lat),Number(original84XY[startPointIndex].long)];
            stopPoint =  [Number(original84XY[startPointIndex+1].lat),Number(original84XY[startPointIndex+1].long)];
            currentPoint = [Number(original84XY[startPointIndex].lat),Number(original84XY[startPointIndex].long)];
       ...
       /*     pmap.getView().fit(feature_tofit.getGeometry().getExtent(),map.getSize(),{
        ...
        //添加历史路线
        scope.addlevelchangeHistoryRouteOfGPSPoint=function (pmap,polylinegeom,original84XY,begintime,endtime) {
            _begintime=begintime;
            _endtime=endtime;
            window.mode="historypath";
            window._original84XY=original84XY;
            ...
            var vectorVertx=new ol.source.Vector({features:[new ol.Feature({
                geometry:new ol.geom.LineString(polylinegeom)
            })
            ]});
            var vectorLayerHistoryVertx = new ol.layer.Vector({
